...
class SmartLogActivity : AppCompatActivity() {
    ...
    private val speechRecognizerLauncher = registerForActivityResult(androidx.activity.result.contract.ActivityResultContracts.StartActivityForResult()) { result ->
        val btnVoiceLog = findViewById<Button>(R.id.btnVoiceLog)
        ...
        if (result.resultCode == android.app.Activity.RESULT_OK) {
            ...
            if (spokenText.isNotEmpty()) {
                processVoiceLogWithAI(spokenText)
            }
        }
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnVoiceLog?.setOnClickListener {
            ...
            intent.putExtra(android.speech.RecognizerIntent.EXTRA_PROMPT, "Speak your log entry...")
            try {
    ...
    private fun processVoiceLogWithAI(spokenText: String) {
        val btnVoiceLog = findViewById<Button>(R.id.btnVoiceLog)
        val originalText = btnVoiceLog.text
        btnVoiceLog.isEnabled = false
        btnVoiceLog.text = "Processing..."
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            try {
                ...
                val prompt = """
                    ...
                    - "company" (string, the company or organization name, null if none)
                    - "jobTitle" (string, the person's job title or role, null if none)
                    - "address" (string, the physical address or location, null if none)
                    - "notes" (string, summary of the interaction)
                    - "category" (string, choose ONE from: "Recruit", "Prospect", "Client", or null)
                    - "subCategory" (string, if category is Client, choose "Investment" or "Life Insurance", else null)
                ...
                withContext(Dispatchers.Main) {
                    applyVoiceLogJson(json)
                    btnVoiceLog.isEnabled = true
                    btnVoiceLog.text = originalText
                }
            } catch (e: Exception) {
                withContext(Dispatchers.Main) {
                    Toast.makeText(this@SmartLogActivity, "AI Error: ${e.message}", Toast.LENGTH_LONG).show()
                    ...
                    etNotes.setText(if (currentNotes.isEmpty()) spokenText else "$currentNotes\n\n[Voice Log]: $spokenText")
                }
            }
        }
    }

    private fun applyVoiceLogJson(json: org.json.JSONObject) {
        val etContactName = findViewById<EditText>(R.id.etContactName)
        val etNotes = findViewById<EditText>(R.id.etQuickNote)
        val etContactEmail = findViewById<EditText>(R.id.etContactEmail)
        val etContactAddress = findViewById<EditText>(R.id.etContactAddress)
        ...
        if (json.has("name") && !json.isNull("name")) {
            val extractedName = json.getString("name")
            if (extractedName.isNotEmpty()) {
                etContactName.setText(extractedName)
                contactName = extractedName
            }
        }
        
        if (json.has("phone") && !json.isNull("phone")) {
            findViewById<TextView>(R.id.tvLogPhoneNumber).text = json.getString("phone")
            phoneNumber = json.getString("phone")
        }
        
        if (json.has("email") && !json.isNull("email")) {
            etContactEmail.setText(json.getString("email"))
        }
        
        if (json.has("company") && !json.isNull("company")) {
            val c = json.getString("company")
            if (c.isNotEmpty() && etContactCompany.text.isNullOrEmpty()) {
                etContactCompany.setText(c)
            }
        }
        
        if (json.has("jobTitle") && !json.isNull("jobTitle")) {
            val t = json.getString("jobTitle")
            if (t.isNotEmpty() && etContactJobTitle.text.isNullOrEmpty()) {
                etContactJobTitle.setText(t)
            }
        }
        
        if (json.has("address") && !json.isNull("address")) {
            val a = json.getString("address")
            if (a.isNotEmpty() && etContactAddress.text.isNullOrEmpty()) {
                etContactAddress.setText(a)
            }
        }
        
        if (json.has("notes") && !json.isNull("notes")) {
            val extractedNotes = json.getString("notes")
            if (extractedNotes.isNotEmpty()) {
                val currentNotes = etNotes.text.toString()
                etNotes.setText(if (currentNotes.isEmpty()) extractedNotes else "$currentNotes\n\n[Voice Log]: $extractedNotes")
            }
        }
        ...
        if (json.has("subCategory") && !json.isNull("subCategory")) {
            val subCat = json.getString("subCategory")
            cbInvestment.isChecked = subCat.contains("Investment", true)
            cbLifeInsurance.isChecked = subCat.contains("Life Insurance", true)
        }
        ...
        if (json.has("targetMarket") && !json.isNull("targetMarket")) {
            ...
            for (i in 0 until tmArray.length()) {
                ...
                if (trait.contains("Children", true) || trait.contains("Kids", true)) cbTmChildren?.isChecked = true
        ...
        // Auto-expand advanced section so user can see AI-filled fields
        val hasAdvancedData = listOf("email", "address", "company", "jobTitle", "targetMarket", "category")
            .any { key -> json.has(key) && !json.isNull(key) }
        if (hasAdvancedData) {
            val container = findViewById<android.view.View>(R.id.containerAdvancedLogging)
            val toggle = findViewById<android.widget.TextView>(R.id.tvToggleAdvanced)
        ...
        if (json.has("followUpDate") && !json.isNull("followUpDate")) {
            ...
            try {
                ...
                if (parsedDate != null) {
                    ...
                    btnScheduleCustom.text = formattedDate
                    Toast.makeText(this, "Follow-up set for $formattedDate", Toast.LENGTH_SHORT).show()
                }
            } catch (e: Exception) {
                Log.e(TAG, "Failed to parse followUpDate from voice log: $dateStr", e)
            }
        }
        ...
        if (json.has("appointmentDate") && !json.isNull("appointmentDate")) {
            ...
            try {
                ...
                if (parsedDate != null) {
                    ...
                    val summarySdf = SimpleDateFormat("MMM dd h:mm a", Locale.getDefault())
                    findViewById<Button>(R.id.btnMarkAppointment)?.text = "Appt: ${summarySdf.format(parsedDate)}"
                    Toast.makeText(this, "Appointment inferred for ${summarySdf.format(parsedDate)}", Toast.LENGTH_SHORT).show()
                }
            } catch (e: Exception) {
                Log.e(TAG, "Failed to parse appointmentDate from voice log: $dateStr", e)
            }
        }
